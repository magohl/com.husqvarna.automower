...
module.exports = class AutomowerApiUtil {
  ...
  /* Get mower work areas/zones */
  async getMowerWorkAreas(mowerId) {
    try {
      let appkey = this.homey.settings.get('appkey');
      let token = await this.login();
      ...
      let that = this;
      const res = await fetch(`${baseAutomowerApiUrl}/mowers/${mowerId}/workAreas`, requestOptions)
        .then(function(response) {
          if (response.status >= 400 && response.status < 600) {
            that.homey.error(`Server responded with http/${response.status}`);
            throw new Error("Bad response from API. Check credentials and connection and try again!");
        ...
        .then(text => JSON.parse(text))
        .catch(err => {
          this.homey.error('Error fetching work areas:', err);
          throw new Error(err);
        });

      if (res && res.data) {
        const workAreas = res.data.map(area => ({
          id: area.id,
          name: area.attributes.name || `Zone ${area.id}`,
          enabled: area.attributes.enabled !== false,
          cuttingHeight: area.attributes.cuttingHeight || 50
        }));
        
        this.homey.log(`Found ${workAreas.length} work areas:`, workAreas);
        return workAreas;
      }
      
      this.homey.log('No work areas data in API response');
      return [];
      
    } catch (error) {
      this.homey.error('Error fetching work areas from API:', error);
      throw error;
    }
  }
  ...
  async apiCall(method, endpoint) {
    try {
      ...
      let that = this;
      const res = await fetch(`${baseAutomowerApiUrl}${endpoint}`, requestOptions)
        .then(function(response) {
          if (response.status >= 400 && response.status < 600) {
            that.homey.error(`Server responded with http/${response.status}`);
            throw new Error("Bad response from API. Check credentials and connection and try again!");
        ...
        .then(text => JSON.parse(text))
        .catch(err => {
          this.homey.error(`Error in API call ${method} ${endpoint}:`, err);
          throw new Error(err);
        });
      ...
      this.homey.error(`Generic API call failed for ${method} ${endpoint}:`, error);
      throw error;
    }
  }
